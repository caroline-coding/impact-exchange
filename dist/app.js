...
// The site is a static snapshot: the old API routes now resolve to
// prebuilt JSON files under /data (see export-static.js), with search and
// leaderboard-sort handled client-side.
...
async function api(path) {
  let m;
  if (path === '/api/orgs') return fetchData('/data/orgs.json');
  if ((m = path.match(/^\/api\/leaderboard\?sort=(\w+)$/)))
    return (await fetchData('/data/leaderboard.json'))[m[1]];
  if ((m = path.match(/^\/api\/orgs\/(\w+)\/(trades|holders|comments)$/)))
    return (await fetchData(`/data/orgs/${m[1]}.json`))[m[2]];
  if ((m = path.match(/^\/api\/users\/search\?q=(.*)$/))) {
    const q = decodeURIComponent(m[1]).trim().toLowerCase();
    const all = await fetchData('/data/users.json');
    return q ? all.filter((u) => u.name.toLowerCase().includes(q)) : all;
  }
  if ((m = path.match(/^\/api\/users\/(\d+)\/(profile|history)$/)))
    return (await fetchData(`/data/users/${m[1]}.json`))[m[2]];
  if ((m = path.match(/^\/api\/tags\/(\w+)\/(profile|history)$/)))
    return (await fetchData(`/data/tags/${m[1]}.json`))[m[2]];
